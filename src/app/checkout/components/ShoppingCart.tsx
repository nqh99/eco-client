  ...
  return (
    <div className="flex flex-col gap-3 text-sm">
      ...
      {[...productsByBrand.values()].map((data, index) => {
        return (
          <div key={index} className="rounded-lg bg-white px-2 text-sm">
            <div className="flex items-center gap-3 border-b-[0.5px] py-2">
              <Checkbox
                id={data.brand?.id || ''}
...
const ShoppingCartHeader = ({
  ...
  return (
    <div className="grid grid-cols-12 items-center gap-1 rounded-lg bg-white p-2 text-sm">
      <div className="col-span-6">
        ...
        >
          <Checkbox.Indicator />
          <Checkbox.Label className="ml-2 select-none text-informal">
            Tất cả ({cal.getTotalQty()} sản phẩm)
          </Checkbox.Label>
        </Checkbox>
      </div>
      <div className="col-span-6 grid grid-cols-12 items-center justify-items-center gap-1 text-center text-informal">
        <span className="col-span-3 block">Đơn giá</span>
        <span className="col-span-5 block">Số lượng</span>
        <span className="col-span-3 block">Thành tiền</span>
        <div className="col-span-1 justify-self-end">
          <Button
            onClick={() => setShowDeleteAllPop(true)}
            className="group flex-shrink rounded-md border bg-[#ebf1f5] p-1 hover:border-primary"
          >
...
const DiscountVoucher = () => {
  ...
  return (
    <div className="flex items-center gap-3 border-t-[0.5px] py-2">
      <Button
      ...
      {selectedVoucher ? (
        <div className="flex items-center space-x-2 text-orange-500">
          <div className="relative inline-flex items-center justify-center rounded-[5px] border border-orange-500 bg-[#fff5e8] px-5 py-[2px] text-orange-500">
            <span className="font-medium">
              {/* TODO: enhance later */}
              Đã giảm{' '}
            </span>
            <div className="absolute -left-[0.05rem] top-1/2 h-3 w-2 -translate-y-1/2 transform rounded-r-full border border-l-0 border-orange-500 bg-[#fff5e8]"></div>
            <div className="absolute -right-[0.08rem] top-1/2 h-3 w-2 -translate-y-1/2 transform rounded-l-full border border-r-0 border-orange-500 bg-[#fff5e8]"></div>
          </div>
        </div>
      ) : (
